...
public class ArmControllerScript : MonoBehaviour {
	...
	Dictionary<string, ImpactStrategy> impactStrategies = new Dictionary<string, ImpactStrategy> ();
	...
	void Start () {
		...
		playerUI = playerManager.GetComponent<PlayerUI> ();
		// playerUI.SetCurrentAmmoText (currentAmmo);
	}
	...
	void Update () {
		...
		if (Input.GetMouseButtonDown (0) && !ShootSettings.automaticFire && isDisableShooting()) {
		...
		for (int i = 0; i < ShootSettings.pellets; ++i) {
			...
			if (Physics.Raycast (Spawnpoints.bulletSpawnPoint.transform.position, direction, 
				...
				if (hit.rigidbody != null) {
					impactStrategies [hit.transform.tag].makeImpact (prefabs, hit);
				}
			}    
		}
	}
	...
	void AnimationCheck () {
		...
		enableMinigunShooting = anim.GetCurrentAnimatorStateInfo (0).IsName ("Spin");
